...
def main():
    ...
    # 2. Predict Endpoint
    print_section("Testing POST /predict (percentile=87, category=GOPENS, CS)")
    payload = {
        "percentile": 87.0,
        "category": "GOPENS",
    ...
    if resp.status_code == 200:
        ...
        print("SUCCESS: /predict endpoint working.")
    else:
        print(f"FAILURE: Status {resp.status_code}, detail: {resp.text}")

    # 3. College Detail Endpoint
    print_section("Testing GET /college/6271 (PICT Pune)")
    ...
    if resp.status_code == 200:
        ...
        print("SUCCESS: /college/{college_code} endpoint working.")
    else:
        print(f"FAILURE: Status {resp.status_code}, detail: {resp.text}")

    # 4. College Search Endpoint
    print_section("Testing GET /search?q=COEP")
    ...
    # 5. Invalid Percentile Range Validation
    print_section("Testing POST /predict with invalid percentile (200)")
    payload = {
        "percentile": 200.0,
        "category": "GOPENS",
        "branch": "Computer Engineering",
